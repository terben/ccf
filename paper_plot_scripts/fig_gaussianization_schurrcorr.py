#!/usr/bin/env python3
"""Comparison variant of figure_gaussianization.py, using schurcorr's
public API throughout instead of the local, hand-vectorized
r_to_alpha / alpha_to_r helpers.

This is NOT the script that reproduces the published Fig. 3 / Table
tab:qg_moments -- see figure_gaussianization.py and its docstring for
why the local helpers are kept there. This script exists purely so the
two can be run side by side and the numerical differences inspected
directly, to decide whether they are small enough to switch the
"official" script over to schurcorr.pacf / schurcorr.from_pacf as well.

Two differences from figure_gaussianization.py to expect:

1. Numerics. schurcorr.pacf/from_pacf on a 2-D batch loop over samples
   in Python, calling the same row-by-row recursion as the 1-D case
   (see schurcorr/levinson.py and Step 3 of the roadmap). This script's
   local r_to_alpha/alpha_to_r instead vectorize the same recursion
   across the sample axis via np.sum(..., axis=1). Both are correct
   implementations of the same recursion; they differ at the
   floating-point-rounding level (a handful of ULP per step, which can
   compound over N=15 lags and M=400000 samples). Compare the two
   scripts' printed roundtrip errors and D_KS/skewness/kurtosis rows,
   and diff the two stats tables, to judge whether this matters here.

2. Performance. The batched schurcorr.pacf/from_pacf calls are a
   Python-level loop over M samples (one call to the scalar recursion
   per row), rather than a vectorized NumPy pass over all samples at
   once per lag. Expect this script to run measurably slower than
   figure_gaussianization.py at the default M=400000; both scripts
   print wall-clock timings for the r<->alpha conversions to make this
   visible.

3. Admissibility handling. schurcorr.pacf(r) raises SingularToeplitzError
   or ValueError immediately if any row's recovered alpha leaves
   (-1, 1) (see Step 2 of the roadmap); the local r_to_alpha never
   raises, and figure_gaussianization.py instead checks
   max(abs(alpha_direct)) after the fact. For the realistic, lag-decaying
   correlations simulated here this is not expected to trigger, unlike
   the adversarial near-boundary sampling in figure_roundtrip.py's Panel
   A -- but it is now a real possibility on every run rather than a
   silent near-miss, so this script reports it explicitly if it happens.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output", "-o", type=Path,
                   default=FIGDIR / "fig_4_qg_transport_schurrcorr.pdf",
                   help="output PDF path "
                        "(default: ../figs/fig_4_qg_transport_schurrcorr.pdf)")
